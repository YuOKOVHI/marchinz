...
**GitHub Actions が毎日 06:00 JST に自動で回ります**
（`.github/workflows/videos-daily-update.yml`、macOSランナー）。

| 対象 | 反映 |
|---|---|
| **海外**（FloMarching） | **全自動**。「10分以上 かつ タイトル/サムネイルに FULL SHOW」という機械的な規則だから |
| **POV（確実な線）** | **自動**。収録実績のあるチャンネル × 題名にPOV語 × 楽器名 × 団体名 × 5〜20分 |
| **POV（それ以外）** | **Issue を立てて人が決める** |
| マーチング等・MIX3 | 触らない（手動のまま） |

> **なぜ macOSランナーなのか**: 海外の採用15件のうち **10件(67%)はサムネイルOCRでしか
> 判定できず**、OCRは macOS Vision を使います。Linuxで走らせるとルールが黙って弱くなり、
> 「更新なし」に見えてしまいます。
>
