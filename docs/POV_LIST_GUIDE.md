...
- `age_limit >= 18`（18歳以上の年齢制限）は採用しません。
- メンバー限定、購読者限定、プレミアム限定、非公開、ログイン・認証必須の動画は採用しません。
- YouTubeメタデータで制限情報が取得できない場合は、制限ありと推測して除外せず、従来のPOV判定を続けます。誤除外を避けるためです。
...
奏者POVを元にした譜面・採譜動画は、説明欄などに元POV動画への参照があり、
マーチングの演奏内容を具体的に確認できるものに限りPOV分類へ収録できます。
通常の奏者POVと混同しないよう、表示名の末尾へ必ず `【譜面】` を付けます。
一般的な楽譜解説・演奏者視点を伴わない採譜動画は対象外です。

---

## 更新履歴

- 2026-08-17 収集失敗を「新着なし」と誤判定しない終了コードと、譜面・採譜動画の収録基準を追加
