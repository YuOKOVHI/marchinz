...
|----|------|------|
| 1.38.4 | 2026-07-19 | **プロレビュー(シニアエンジニア+UI/UXデザイナー)の指摘を反映**。[高3件] ①**`[hidden]` がサイト本体で効いていなかった** — `.user-profile-tab{display:inline-flex}` 等のオーサー宣言がUAの `[hidden]{display:none}` に常に勝つため、`el.hidden = true` が無効。**本人限定の「MarchinZ Days」タブが他人・ゲストにも表示されていた**(実測: `#prof-tab-base` は `hidden=true` でも `display:flex`)。`styles.css` 先頭に `[hidden]{display:none!important}` を追加し、訪問者向け非表示の列挙にも `#prof-tab-base`/`#prof-pane-base` を追加。②**プレビュー専用の重ね描きが実時間録画に焼き込まれていた** — `exportRealtime` はプレビューcanvasをそのまま録画するため、WebCodecs非対応環境(=iOS Safari、最優先ターゲット)で「カメラN」バッジと範囲外の案内が**恒久的に焼き込まれたMP4**が出ていた。`MC.preview.overlayOn` を新設し録画中はオフ、`finally` で必ず復帰。③先行ペイントが①の露出時間を延ばす問題は①の修正で緩和。[中8件] カレンダー照合表の**失敗結果を5分キャッシュしない**+`finally` で inflight を必ず解放、ログ蓄積を `MC.pushDebug` に一本化して全経路400件で打切り(未捕捉エラー経路が無制限だった)、`showErrorLog` を try/catch で保護し数値を正規化(メタデータ未確定クリップで報告機構自体が落ちる問題)、`runEasy` 実行中は同期/カット割/カラー/書き出しを一括で無効化(`MC.ui.setBusy`)+失敗時も `renderAll` で表示を整合、`sitechrome.css` に自前の `[hidden]` ガード、rAF保険を再生中のみに限定し `_lastTick` の更新順を是正。[低] 未使用の重複 `profile` 定義を削除、モバイルタブを `auto-fit` 化、アイコン指定をメディアクエリ内へ、ドロワーの `overflow` を元値へ復帰、上限文言を `MZ_LIMITS` から生成、シーク時に `_lastCurId` をリセット。[デザイナー] **ツール3本のモバイルヘッダーが本体と不一致**(タグライン2行+ボタン+ナビ横スクロールで画面の45%を占有)→ 本体と同じく719px以下ではハンバーガー+ドロワーへ集約(「映像ツール」節でツール間の行き来も確保)。 |
| 1.38.3 | 2026-07-19 | **マイページのスマホ最適化+ツールのヘッダー/フッター整理**(優さん指示3件)。①**ハンバーガーメニューに MarchinZ Switcher(映像制作)** を「MarchinZ Days(練習記録)」の直上へ追加(アイコンは `fa-clapperboard`、Daysの `fa-drum` と同じFAモノクロで統一)。②**マイページのタブUIを全面刷新(719px以下)** — 小さな文字が7個並ぶ横スクロール帯をやめ、**アイコン+ラベルの3列グリッドチップ**へ(各104×60px以上のタップ領域、件数はチップ右上のアンバー丸バッジ、選択中はネイビー反転)。通知・運営よりは2列の大きなチップに。**横スクロール自体を廃止したため、v1.38.2で対処した「iPhoneで上下に揺れる」問題も構造的に解消**。ラベルも「大会動画マイリスト」→「大会動画」等に短縮。③**3ツールのヘッダー/フッターをYouTubeページと同じサイト標準に** — フッターから「マーチンズ/MarchinZで、できること。」セクションと「つづけて使う」(ツール相互リンク)を削除し、バナー行+3列リンク集+コピーライトのみに。「無料・すべて端末内で処理します」の帯は**ページ下部(フッター直前)へ移動**。 |
| 1.38.2 | 2026-07-19 | **マイページの表示速度と操作感の改善**(優さん報告2件)。①**先行ペイント** — `loadAndRenderCore` がヘッダー描画前に8つの一覧クエリ(Log最大800件・ブックマーク200×2・リスト80×2・カレンダー500件・通知80件・Note全件)の完了を待っていたのが体感の遅さの主因。プロフィール文書が届いた時点でヘッダー/カバー/練習記録(MarchinZ Days)を即描画・即マウントし、一覧は「一覧を読み込み中…」を出して並行取得に変更。**練習記録(Days)は一覧クエリと並行で読み込まれる**ため、右上メニュー→練習記録の体感も同時に改善。②旧「マイリスト」既定リストの掃除(移行処理)を非ブロック化。③カレンダー照合表(500件クエリ)を**5分キャッシュ+実行中デデュープ**。④**タブ帯の上下揺れを解消** — `.user-profile-tabs` の `touch-action` が `pan-x pan-y` で縦パンを許可していたため、iPhoneで横スクロール中に指の縦ブレでページが上下に揺れていた。`pan-x` のみに固定+`overscroll-behavior: contain`(縦に動かしたいときはタブの外を触る)。`user-profile-page.js`/`styles.css`/`marchinz-brushup.css` の `?v=` を1.38.2へ更新。 |
| 1.38.1 | 2026-07-19 | **Switcher: 初見でも迷わない導線へ**(優さん指示5件)。①**エラーのログを画面で見せる** — `MC.debug` にログを蓄積(未捕捉エラー・Promise拒否も収集)し、書き出し/カット割の失敗時に「詳しいログ」パネルを展開。版番・UA・書き出し方式・素材情報・スタックを含む本文を**ワンタップでコピー**でき、そのまま不具合連絡に貼れる。②**使える条件を1行に** — 表をやめ、ゲストには「ゲストは5分まで、登録ユーザーは12分まで。無料登録」、ログイン済みには「12分まで使えます。」だけを出す(自分に関係ない情報を読ませない)。③**「おすすめ / こだわり」タブ** — 素材を入れると出現。初期は**おすすめ**で「おすすめで開始」ボタン1つ(同期→自動カット割→カラーマッチを一気に実行)。細かい設定は**こだわり**タブへ格納。④**プレビューを上部に固定** — PC/スマホとも `.stage` を sticky にし、下へスクロールしても映像と再生バーが見えたまま。⑤**自動スイッチングは横型のみ** — 比率から9:16を削除し、モード説明の「横型・縦型が選べます」も撤去。あわせて**`[hidden]{display:none!important}` ガードをSwitcher/ReAngleへ追加**(タブ実装時に `.pro-pane{display:flex}` が `[hidden]` に詳細度で勝ち、非表示が効かずレイアウトが崩れた。Privacyで過去に起きた事故と同じ罠)。検証: 合成3カメラで「おすすめで開始」が同期(オフセット理論値一致)→カット割3つ→カラーマッチまで通ること、タブ切替・sticky(1280/375px)・横はみ出しなしを実測。 |
| 1.38.0 | 2026-07-19 | **マイナー: クリエイターツール3本をサイトの一部として統合**(優さん指示4件)。①**「使える条件」表を明示**(`tools/shared/limits.js` の `renderPlanNote`) — ゲスト/無料登録で何がどこまで使えるか(動画5分/12分・写真1枚/5枚)を各ツールの取り込み画面に表で常設し、**いま自分がどちらなのか**も印で示す。「機能はどちらも同じ・上限だけが違う」ことと、上限の理由(端末負荷)も明記。差し込み先は `[data-mz-plan]`。②**上限超過・書き出し失敗のメッセージを実用的に** — 上限超過は「約N分です。ゲストは5分・無料登録で12分まで使えます。見せたい場面だけ短く切り出してからお試しください」に。書き出し失敗は `MC.ui.exportFailHint` で原因別の対処(メモリ不足→範囲を短く/GPU上限→サイズを下げる/コーデック非対応→別ブラウザ/長時間→IN・OUTで区切る)を出し、技術詳細も併記して問い合わせしやすく。③**ヘッダーをTOPと同じグローバルメニューに**+④**フッターにTOPの「MarchinZで、できること。」以下を実装** — 3ツール共通の `tools/shared/sitechrome.js` / `sitechrome.css` を新設し、ブランドヘッダー(ロゴ+タグライン+はじめての方/ログイン)、グローバルナビ(TOP/コミュニティ/大会動画/YouTube/メディア/クリエイター/運営)、「できること」3カード、広告バナー行、3列リンク集、ツール相互リンク、コピーライト(版番付き)を注入。ツールはSPAではないためリンクは絶対パス(`/#...`)。本体CSSは読み込まず `mzsc-` 接頭辞で自己完結(Switcherの白基調・ReAngleのダークどちらの上でも読める配色)。375px横はみ出しなしを実測。 |
| 1.37.2 | 2026-07-19 | **Switcher: プレビューが止まって「スイッチングされない」問題の修正**(優さん報告)。①**rAF停止時の保険を追加**(`tools/switcher/js/preview.js`) — 描画ループが `requestAnimationFrame` のみに依存しており、rAFが発火しない環境(バックグラウンドタブ・一部の埋め込みブラウザ)では**再生してもタイムラインの時刻が進まず、画面が黒いまま一切切り替わらない**。ReAngleと同じ `setInterval` フォールバック(250ms停滞を検知して tick を回す)を実装。実測: 修正前は再生20秒間 t=0 固定・真っ黒、修正後は t が正常進行しカメラが3→6→7本(形状判定)で切り替わることを確認。②**カット切替直後の静止をなくす** — 切り替わった素材がまだ再生開始されていない場合、従来は `driftFix`(500ms間隔)を待つため最大0.5秒が静止画に見えた。カット変化を検知して即 `ensurePlaying` するように。③**プレビューに「カメラN」バッジを常時表示**(カット割モードのみ、左下・プレビュー専用で書き出しには焼き込まない) — カラー自動マッチが全カメラの色を揃えるため、アングルの似た実素材では切り替わりが判別しづらい。いま何番目の素材が映っているかを常に見せる。 |
| 1.37.1 | 2026-07-19 | **クリエイターツールの磨き込み(優さん指示5件+検証)**。①**ウォーターマーク透過化+右上端寄せ** — `tools/shared/marchinz-wm.png` がJPEG由来の白背景付き(全ピクセル不透明)で、書き出し/プレビューに白い矩形が出ていた。`logo/logo_font.png` から外周flood-fill+1pxフェザーで背景を透過化した520×173へ差し替え。位置も右上の隅へ(横1920: 右40/上40→右16/上16、縦1080: 右30/上120→右12/上24)。ReAngle/Switcher共通。②**Switcherフィルター初期値** — 保存値の復元を廃止し常に「MarchinZ」で開始。名称も**「MarchinZカラー」**に変更。③**自動スイッチングモードは境界線を初期オフ**(モード選択時に設定、縦型は従来どおりオン)。④**IN/OUTのわかりやすさ改善**(初見ユーザー対応) — ボタンを「ここから書き出す IN」「ここまで OUT」(旗アイコン付き)に、シークバー下にIN〜OUTの**範囲バンド**(旗ラベル付き・全体書き出し時は薄く)、範囲ラベルは「いまは全体を書き出します(INとOUTで絞れます)」/「書き出し範囲 IN → OUT」、**プレビューは範囲外で暗転+「ここは書き出されません」表示**(プレビュー専用・書き出しには入らない)。⑤**TOPのSwitcherカードをPCでウィジェット幅いっぱいに**(2カラムグリッドの半幅→1カラム640px、上下のTODAY/練習ツールと同幅)。あわせて**自動スイッチング機能の動作検証を実施** — 合成3カメラMP4(WebCodecs生成)で波形同期(オフセット理論値一致)→自動カット割(3カメラ巡回)→プレビュー/書き出しMP4の切替を画素判定で確認、機能自体は正常。「切り替わらない」報告の正体はカラー自動マッチによる色の同化、または旧版本番の可能性。 |
| 1.37.0 | 2026-07-19 | **MarchinZ ReAngle: 床自動検出の刷新+「見る位置」機能追加**。①**床自動検出v2**(`tools/reangle/js/detect.js`) — 旧版は「床=画面下部の大面積」前提で、アリーナ撮影(画面下部が観客席、床は画面中段)では観客込みの歪んだ四角を返し、既定90%補正が映像全体を回転・シアーさせる不具合があった。新版は複数の水平バンド(y=0.48〜0.83)から一様な床色を探索(散らばり判定はp60)→二値化→クロージング→flood→凸包→最大内接四角形→**エッジ実在性検証**(辺の内外プローブで領域境界に実在するか判定、奥の縁が実在しない候補は棄却)→**信頼度後処理**(実在しない辺は「補正が効かない形」へ均し、誤った回転・横ずれ補正を防止)。②**「見る位置（左右）」スライダー新設**(Step2) — 出力空間の水平シアーで視点の横移動を再現(±25、右へ動かすとより中央寄りから撮った見え方に)、tilt/縦動画アスペクト比を考慮した実装。③**プロレビュー実施**(サブエージェント=シニアエンジニア役、私=UI/UXデザイナー役)で指摘9件を検出・全反映 — [高]床検出のp60統計計算バグ(median計算のin-placeソートでピクセル対応が破壊)を修正、後処理後の凸性再検証を追加、スマホでスライダー操作中もプレビューが見えるようsticky化。実アリーナ動画4時点+合成体育館フレームで検証、homographyセルフテストOK。書き出し(WebCodecs)のE2E実ファイル確認は未実施。 |
| 1.35.7 | 2026-07-13 | **クリエイターツール〈ベータ版〉に MarchinZ ReAngle を追加(3本目)**。CloudeCode_u/ReAngle を `tools/reangle/` へスタンドアロンHTMLとして配置(Switcher/Privacy と同枠組み・SPA本体の重量増ゼロ)。斜め撮り動画を正面視点へ透視補正(WebGL1フラグメントシェーダの逆マッピング、床4隅→ホモグラフィ Heckbert 閉形式、自動検出+ドラッグ微調整、強度0〜100%既定60%)。書き出しは WebCodecs→mp4-muxer 高速パス+MediaRecorder フォールバック、AACパススルー、iOSは Web Share 保存(Switcher/Privacy と同資産)。カメラ・マイク不使用(ファイル選択→端末内処理→保存のみ)のため Permissions-Policy 変更は不要。ヘッダーを MarchinZ 標準(名前+ベータ版バッジ+「← 戻る」)に差し替え、本文はダークテーマのまま。TOP・#creators のクリエイターツールカードを **ReAngle → Switcher → Privacy** の順に並べ、ReAngle カード(fa-vector-square)を追加。利用ログ(`marchinz-admin-ugc-log.js`)の `TOOL_USE_IDS`・href判定・toolName に `reangle` を配線。netlify.toml に reangle vendor(mp4box/mp4-muxer)の7日キャッシュを追加。実機(iPhone Safari)での保存系・実演技動画の書き出しは未検証=ベータ版表記と整合。 |
| 1.35.0 | 2026-07-12 | **クリエイターツール〈ベータ版〉2本を公開**。CloudeCode_u で構築済みのツールを `tools/` 配下にスタンドアロンHTMLとして配置(SPAフォールバックは実ファイルの Shadowing で共存、SPA本体の重量増ゼロ)。①**MarchinZ Privacy**(`/tools/privacy/`、旧MzMosaic) — 顔自動モザイク(モザイク/ぼかし/黒塗り)。MediaPipe BlazeFace 全ローカル同梱(vendor 約22MB、うちWASM2種21.6MB。SIMD判定で片方≒11MBのみフェッチ)。WebCodecs書き出し+MediaRecorderフォールバック、iOSはWeb Share保存。②**MarchinZ Switcher**(`/tools/switcher/`、旧MarchCut) — ランスルー動画をGCC-PHAT波形同期→縦9:16/横16:9/1:1 × レイアウト8種(縦積み/自動スイッチング/ワイプ等)→MP4。完全端末内処理・無料。両ツールに MarchinZ ヘッダー(名前+ベータ版バッジ+「← MarchinZへ」)。クリエイターページ(#creators)に「クリエイターツール ベータ版」セクション+カード2枚(fa-user-shield/fa-clapperboard)。netlify.toml: `.tflite` MIME明示+tools vendor を7日キャッシュ+**Permissions-Policy の microphone を `(self)` に修正(旧 `()` は自オリジンも拒否し、Days/TOPチューナーのマイクが本番で常に失敗していた隠れバグ)**。実機(iPhone Safari)での保存系は両ツールとも未検証=ベータ版表記と整合。 |
| 1.34.1 | 2026-07-12 | **UX/UIデザイナー+シニアエンジニアの2者レビュー反映**(v1.34.0公開後、デスクトップの実測: TODAY=460px中央ポツン/TOOLSボタン564×88/誘導1140px全幅と幅バラバラ)。[デザイン] ①TODAY・TOOLSランチャー/パネル・誘導カードを**同一のウィジェット幅 `--mz-widget-w: 640px` 中央**に統一(縦一列の「ウィジェットカラム」でリズムを揃える)。②ツール起動ボタンに **chevron(fa-chevron-down)** — 押すと開く手掛かり、開いている間は180度回転。[エンジニア] ③**`[hidden]` vs display:flex バグ修正** — `.mz-dash-countdown` が hidden でも表示され「−日後」の空骨組みが本番露出(実測で発覚)。`.mz-hero-dash`/`.mz-dash-countdown`/`.mz-dash-video` に `[hidden]{display:none!important}` ガード。④**ツール使用中の `mll-auth-changed` で音が止まる問題** — 演奏/チューニング中は誘導カードのみ差し替え、再マウントしない。⑤`loadAndRender` の catch のエラーメッセージ表示に stale ガード(古いロードの例外が新しい画面に出ない)。⑥ダッシュボードの📍カラー絵文字→FA `fa-location-dot`(必須ルール6)。⑦**TOOLS を TODAY'S MARCHINZ ブロック内に統合**(別セクションを廃止し、ダッシュボードの下に「練習ツール」サブ見出し+ランチャーを配置。ウィジェットカラム640pxで一体化)。⑧未ログイン時の誘導文「練習の記録・目標・ストリークは〜」→「練習の記録・目標サポートは、無料登録で使える MarchinZ Days に。」。⑨誘導カードの説明文を撤去しCTAボタンのみに。⑩ツール起動ボタンを縦積み→**横並び(アイコン左・ラベル・chevron右端)**、縦幅88px→52pxに圧縮。 |
