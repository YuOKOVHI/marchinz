...
|----|------|------|
| 1.35.0 | 2026-07-12 | **クリエイターツール〈ベータ版〉2本を公開**。CloudeCode_u で構築済みのツールを `tools/` 配下にスタンドアロンHTMLとして配置(SPAフォールバックは実ファイルの Shadowing で共存、SPA本体の重量増ゼロ)。①**MarchinZ Privacy**(`/tools/privacy/`、旧MzMosaic) — 顔自動モザイク(モザイク/ぼかし/黒塗り)。MediaPipe BlazeFace 全ローカル同梱(vendor 約22MB、うちWASM2種21.6MB。SIMD判定で片方≒11MBのみフェッチ)。WebCodecs書き出し+MediaRecorderフォールバック、iOSはWeb Share保存。②**MarchinZ Switch**(`/tools/switch/`、旧MarchCut) — ランスルー動画をGCC-PHAT波形同期→縦9:16/横16:9/1:1 × レイアウト8種(縦積み/自動スイッチング/ワイプ等)→MP4。完全端末内処理・無料。両ツールに MarchinZ ヘッダー(名前+ベータ版バッジ+「← MarchinZへ」)。クリエイターページ(#creators)に「クリエイターツール ベータ版」セクション+カード2枚(fa-user-shield/fa-clapperboard)。netlify.toml: `.tflite` MIME明示+tools vendor を7日キャッシュ+**Permissions-Policy の microphone を `(self)` に修正(旧 `()` は自オリジンも拒否し、Days/TOPチューナーのマイクが本番で常に失敗していた隠れバグ)**。実機(iPhone Safari)での保存系は両ツールとも未検証=ベータ版表記と整合。 |
