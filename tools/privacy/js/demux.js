...
MZ.MP4Source = class {
  ...
  static _parseTrak(dv, trak) {
    ...
    const dive = (start, end) => {
      for (const b of MZ.MP4Source._children(dv, start, end)) {
  ...
  async init() {
    ...
    if (moovBox && moovBox.size < (64 << 20)) {
      ...
      let feed = dv.buffer;
      const cut = [];
      for (const b of MZ.MP4Source._children(dv, 8, moovBox.size)) {
        if (b.type !== "trak") continue;
        const info = MZ.MP4Source._parseTrak(dv, b);
        if (info.kind !== "soun" || !info.stsd || !info.stsz || !info.stsc || !(info.stco || info.co64)) continue;
        const fmt = MZ.MP4Source._parsePcmEntry(dv, info.stsd);
        if (!fmt) continue;
        const table = MZ.MP4Source._buildPcmChunks(dv, info, fmt);
        this.pcm = {
