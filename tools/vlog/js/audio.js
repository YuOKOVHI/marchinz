...
MV.audio = { SR: 8000, MAX_SEC: 1800 };

/* 線形補間のストリーミングリサンプラ(チャンクをまたいで連続) */
MV.audio.LinearResampler = class {
...
MV.audio.extract8k = async (clip, maxSec = MV.audio.MAX_SEC) => {
  if (clip.audio8k) return clip.audio8k;
  let pcm = null, err1 = null;
  try {
    pcm = await MV.audio.viaRawPcm(clip, maxSec);   // リニアPCM(Resolve等のMOV)は生読みが最速・最軽量
  ...
  if (pcm.length < MV.audio.SR) { clip.hasAudio = false; throw new Error(`音声が短すぎます(1秒未満): ${clip.name}`); }
  clip.hasAudio = true;
  clip.audio8k = pcm;
  clip.stats = MV.audio.stats(pcm);
  return pcm;
};
...
MV.audio.viaRawPcm = async (clip, maxSec) => {
  ...
  const outChunks = [];
  let total = 0;
  const maxFrames = maxSec * MV.audio.SR;
...
MV.audio.viaWebCodecs = async (clip, maxSec) => {
  ...
  const decoder = new AudioDecoder({
    output: ad => {
      try {
        ...
        if (!resampler) resampler = new MV.audio.LinearResampler(ad.sampleRate, MV.audio.SR);
        // モノラルミックスダウン
  ...
  const pcm = new Float32Array(Math.min(total, maxSec * MV.audio.SR));
  let o = 0;
  for (const a of outChunks) {
    const n = Math.min(a.length, pcm.length - o);
    if (n <= 0) break;
...
MV.audio.viaDecodeAudioData = async (clip, maxSec) => {
  const limit = MV.isIOS ? 3e8 : 1.2e9;
  if (clip.size > limit) throw new Error(`ファイルが大きすぎます(${Math.round(limit / 1e8) / 10}GB超)`);
  const ab = await clip.file.arrayBuffer();
  const ctx = new OfflineAudioContext(1, MV.audio.SR, MV.audio.SR);  // 8kHzへ自動リサンプル
  const buf = await ctx.decodeAudioData(ab);
  const d = buf.getChannelData(0);
  const n = Math.min(d.length, maxSec * MV.audio.SR);
  return Float32Array.from(d.subarray(0, n));
};

/* 音質統計: 有音部分の代表RMSとクリッピング率 */
MV.audio.stats = pcm => {
  const win = MV.audio.SR / 2;  // 0.5秒窓
  const rmsList = [];
  let clipped = 0;
...
MV.audio.recommend = () => {
  const cands = MV.S.clips.filter(c => c.stats);
