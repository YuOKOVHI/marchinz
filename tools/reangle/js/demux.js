...
RA.MP4Source = class {
  ...
  static _parseTrak(dv, trak) {
    ...
    const dive = (start, end) => {
      for (const b of RA.MP4Source._children(dv, start, end)) {
  ...
  async init() {
    ...
    mp4.onError = e => console.warn("[RA] mp4box:", e);
    ...
    if (moovBox && moovBox.size < (64 << 20)) {
      ...
      let feed = dv.buffer;
      const cut = [];
      for (const b of RA.MP4Source._children(dv, 8, moovBox.size)) {
        if (b.type !== "trak") continue;
        const info = RA.MP4Source._parseTrak(dv, b);
        if (info.kind !== "soun" || !info.stsd || !info.stsz || !info.stsc || !(info.stco || info.co64)) continue;
        const fmt = RA.MP4Source._parsePcmEntry(dv, info.stsd);
        if (!fmt) continue;
        const table = RA.MP4Source._buildPcmChunks(dv, info, fmt);
        this.pcm = {
  ...
  static findTag5(desc) {
    ...
    for (const d of (desc.descs || [])) {
      const r = RA.MP4Source.findTag5(d);
      if (r) return r;
    }
    return null;
  }
