...
0. **`git push` / デプロイは毎回かならず確認を取る**（2026-07-20 追加）
   - **push は1回ごとに Netlify のビルドクレジットを消費する**。2026-07-12 に
     上限到達でサイトが一時停止した実績がある
   - 「pushして」と言われた**その1回だけ**が許可。同じ会話の続きでも、
     次の push はあらためて確認する（前の許可は次に持ち越さない）
   - コミットは都度作ってよい。**push だけを止める**
   - 2026-07-20、この確認を省いて6回連続で push した。ルールは
     「コマンド早見（実行禁止）」に元からあったが埋もれていたため筆頭へ移した
...
**毎回かならず確認を取ること。1度の許可は1度の実行にしか使えない。**

```bash
git push              # ← Netlify のビルドクレジットを消費する。最重要
netlify deploy --prod
firebase deploy ...
gh workflow run ...   # ← これもビルドを誘発する
```
