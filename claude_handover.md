...
### 4.1 単一フロー（2026-07-08 改訂: 三層乖離は v1.29.1 の push で解消）

```
┌─────────────────────────────────────────────────────────────┐
│ GitHub origin/main = 唯一の正（実装+データ）                  │
│   - 開発: ローカルで編集→検証→ git push origin main          │
│   - データ: YouTube daily workflow が毎朝 commit/push         │
└───────────────────────────┬─────────────────────────────────┘
                            │ push をトリガに自動ビルド
┌───────────────────────────▼─────────────────────────────────┐
│ 本番 https://marchinz.netlify.app（Netlify GitHub 連携）      │
└─────────────────────────────────────────────────────────────┘
```

- 「pull すれば本番と同じ」「push すれば本番が更新される」——**どちらも正しい**のが新運用
- 旧運用の記述（ローカルが正/手動 netlify deploy/main は古い JS）を他ドキュメントで見かけたら歴史的記述として読むこと
